  ...
  const sections = [
    ...
    },
    {
      ...
      steps: [
        ...
        {
          title: 'Scope a token for CI: the CI Trigger role',
          text: 'Every token has a role: Standard user (full read/write), Administrator (also manages tokens/settings), or CI Trigger -- restricted to launching Job Selections/Execution Sequences and reading run results, nothing else. To create one: 1) Config -> Security -> + Add User Access. 2) Name it for the pipeline (e.g. gitlab-nightly-recon). 3) Set Role to CI Trigger (launch + read-only). 4) Copy the raw token now -- it is shown once. 5) Store it as ATOM_API_TOKEN in your CI system, masked/protected. A CI Trigger token gets 403 on Config, Jobs, other Tokens, Settings, Schedules, and the audit log -- even though it is a completely valid token. This is enforced once, centrally, for every request, so a future endpoint can never accidentally skip the check.',
          where: 'Config -> Security -> + Add User Access -> Role',
          when: 'Setting up any CI/CD pipeline token -- prefer this over a Standard user token whenever the pipeline only needs to launch and read results.',
          tip: 'A leaked CI Trigger token cannot read Config rows (database/BO connection credentials) -- a leaked Standard or Administrator token can. Prefer CI Trigger for every pipeline unless it genuinely needs broader access.',
        },
        {
    ...
    },
    {
      ...
      steps: [
        ...
        {
          title: 'Gate CI/CD pipelines from a Job Selection or Sequence',
          text: 'Steps: 1) Save a Job Selection (Launch tab) or Execution Sequence (Sequences tab) with the jobs you want gated; run it once manually first. 2) Click its CI/CD button. 3) In the modal, STEP 1 creates a CI Trigger API token (Config -> Security), STEP 2 lists the ATOM_API_URL / ATOM_API_TOKEN variables to add in GitLab (Settings -> CI/CD -> Variables, mark Masked+Protected), STEP 3 sets source/target env (target env only needed for a dual-env job type like reconciliation), STEP 4 shows the ready-to-copy .gitlab-ci.yml snippet with pip install etl-framework already included. 4) Push -- the pipeline stage runs ./scripts/ci/run-atom-target.sh <selection|sequence> <id> <env>, which launches the run, waits for it, and exits non-zero on FAILED/ERROR/CANCELLED so the pipeline stage fails with it. Works the same for GitHub Actions/Jenkins by calling the atom CLI directly instead of the wrapper script (see docs/cli.md for the full flag reference and exit-code table).',
          where: 'Launch tab / Sequences tab -> CI/CD button',
          when: 'Blocking broken data pipelines from promoting to production.',
        },
        {
          title: 'GitLab Commit Status & sticky MR comments',
          text: 'Once the pipeline above is set up, nothing else to configure -- run-atom-target.sh reports back to GitLab automatically using the job\'s own CI_JOB_TOKEN (injected by GitLab, never something you set). Every pipeline (branch or MR) gets a commit status check named atom/selection/<slug> or atom/sequence/<slug>, going pending -> success/failed/canceled with a description like "3 passed, 1 failed, 0 error"; two different targets on the same commit get their own distinctly-named checks. Merge-request pipelines additionally get one sticky comment per target carrying the run\'s markdown summary -- pushing again updates the same comment in place rather than adding a new one. Both are best-effort: if the GitLab API call fails, the job log shows a warning but the pipeline\'s own pass/fail result is never affected by it.',
          where: 'GitLab merge request / commit status widget -- nothing to configure in Atom itself',
          when: 'Wanting pass/fail visible in GitLab\'s own UI, not just the pipeline log.',
          tip: 'If nothing appears: check the job log for "warning: GitLab CI environment not detected" (pipeline isn\'t actually running in GitLab CI) versus an HTTP warning (the commit-status/MR-comment call itself failed, usually a CI_JOB_TOKEN permission issue) -- either way the run\'s own pass/fail is unaffected.',
        },
        {
